...
void CVarManager::RegisterConsoleObject(const char* pName, IConsoleObject* pObject)
{
    ...
    if (gCvarMap.find(lowerName) == gCvarMap.end())
    {
        ...
        std::sort(gConsoleObjects.begin(), gConsoleObjects.end(), [](IConsoleObject* pA, IConsoleObject* pB) {
            return strcmp(pA->GetName(), pB->GetName()) < 0;
        });
    }
}

bool CVarManager::Execute(const char* pCommand, char delimiter)
{
	char cmdLower[1024];
	...
	char buffer[1024];
	int numArgs = CString::SplitString(cmdLower, buffer, &argList[0], delimiter);
	if (numArgs > 0)
	{
		auto it = gCvarMap.find(argList[0]);
		if (it != gCvarMap.end())
		{
...
void ImGuiConsole::Update(const ImVec2& position, const ImVec2& size)
{
    ...
    if (m_ShowConsole)
    {
        ...
        if (ImGui::Begin("Output Log", nullptr, ImGuiWindowFlags_NoResize | ImGuiWindowFlags_AlwaysAutoResize | ImGuiWindowFlags_NoMove | ImGuiWindowFlags_NoSavedSettings))
        {
            ...
            if (ImGui::InputText("##some thing", m_Input.data(), m_Input.size(), inputFlags, inputCallback, this))
            {
                if (m_Input[0] != '\0')
                {
                    if (strcmp(m_Input.data(), "help") == 0)
                    {
                        E_LOG(Info, "Available Commands:");
                        CVarManager::ForEachCvar([](IConsoleObject* pObject) {
            ...
            if (!m_Suggestions.empty())
            {
                ...
                if (ImGui::Begin("Suggestions", nullptr, ImGuiWindowFlags_NoDecoration | ImGuiWindowFlags_NoFocusOnAppearing | ImGuiWindowFlags_AlwaysAutoResize))
                {
                    for (uint32_t i = 0; i < (uint32_t)m_Suggestions.size(); i++)
                    {
                        if (ImGui::Selectable(m_Suggestions[i], i == (uint32_t)m_SuggestionPos))
                        {
                            m_SuggestionPos = i;
                            strncpy_s(m_Input.data(), std::size(m_Input), m_Suggestions[i], strlen(m_Suggestions[i]));
                            m_Suggestions.clear();
