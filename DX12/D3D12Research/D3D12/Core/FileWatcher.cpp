...
bool FileWatcher::StartWatching(const char* pPath, const bool recursiveWatch)
{
    QueryPerformanceFrequency(&m_TimeFrequency);

    if (!Paths::DirectoryExists(pPath))
    {
        E_LOG(Error, "Directory '%s' does not exist. Failed to start file watcher.", pPath);
        return false;
    }
        ...
        while (GetQueuedCompletionStatus(m_IOCP, &numBytes, &key, &ov, INFINITE))
        {
            ...
            while (offset < numBytes)
            {
                ...
                if (pWatch->Changes.size() > 0)
                {
                    ...
                    add = prevEvent.Path != newEvent.Path || prevEvent.EventType != newEvent.EventType;
                }
