#ifndef __RAYTRACINGCOMMON_INCLUDE__
#define __RAYTRACINGCOMMON_INCLUDE__
...
MaterialProperties GetMaterialProperties(uint materialIndex, float2 uv, float mipLevel)
{
...
float3 OffsetRay(float3 position, float3 geometryNormal)
{
    ...
    float3 p_i = float3(
        asfloat(asint(position.x) + ((position.x < 0.0f) ? -of_i.x : of_i.x)),
        asfloat(asint(position.y) + ((position.y < 0.0f) ? -of_i.y : of_i.y)),
        asfloat(asint(position.z) + ((position.z < 0.0f) ? -of_i.z : of_i.z))
    );

    return float3(
        (abs(position.x) < origin) ? position.x + float_scale * geometryNormal.x : p_i.x,
