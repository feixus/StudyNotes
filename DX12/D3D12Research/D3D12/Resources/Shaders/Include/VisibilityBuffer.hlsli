...
GradientInterpolationResults InterpolateAttributesWithGradient(float2x3 attributes, float3 db_dx, float3 db_dy, float2 d, float2 pTwoOverRes)
{
    ...
    result.Interp = attribute_s + attribute_x * d.x + attribute_y * d.y;
    return result;
}

float InterpolateAttribute(float3 attributes, float3 db_dx, float3 db_dy, float2 d)
{
    float attribute_x = dot(db_dx, attributes);
    float attribute_y = dot(db_dy, attributes);
    float attribute_s = attributes[0];

    return attribute_s + attribute_x * d.x + attribute_y * d.y;
}

float2 Interpolate2DAttribute(float2x3 attributes, float3 db_dx, float3 db_dy, float2 d)
{
    float3 attr0 = attributes[0];
    float3 attr1 = attributes[1];
    float2 attribute_x = float2(dot(db_dx, attr0), dot(db_dx, attr1));
    ...
    float2 result = attribute_s + attribute_x * d.x + attribute_y * d.y;
    return result;
}

float3 Interpolate3DAttribute(float3x3 attributes, float3 db_dx, float3 db_dy, float2 d)
{
