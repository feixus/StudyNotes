...
bool EvaluateIndirectBRDF(int rayType, float2 u, BrdfData brdfData, float3 N, float3 V, float3 geometryNormal, out float3 direction, out float3 weight)
{
    ...
    float3 directionLocal = float3(0, 0, 0);
    ...
    else if (rayType == RAY_SPECULAR)
    {
        ...
        if (alpha == 0.0f)
        {
            ...
            // also prevents NaNs appearing due to divisions by zeros
            Hlocal = float3(0, 0, 1);
        }
        else
        {
            // for non-zero roughness, VNDF sampling for GG-X distribution
            Hlocal = SampleGGXVNDF(Vlocal, alpha.xx, u);
        }
        ...
        float3 energyCompensation = EnergyCompensationFromMultipleScattering(brdfData.Specular, brdfData.Roughness, NdotV);
...
void RayGen()
{
    ...
    for (int i = 0; i < cViewData.NumBounces; i++)
    {
        ...
        // we must correctly weight the sample based on this elimination to account for bias
        if (i > MIN_BOUNCES)
        {
