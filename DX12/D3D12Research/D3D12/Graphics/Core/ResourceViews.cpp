...
void ShaderResourceView::Create(GraphicsBuffer* pBuffer, const BufferSRVDesc& desc)
{
    ...
    if (EnumHasAnyFlags(bufferDesc.Usage, BufferFlag::AccelerationStructure))
    {
        ...
        {
            m_Descriptor = pBuffer->GetGraphics()->AllocateDescriptor<D3D12_SHADER_RESOURCE_VIEW_DESC>();
        }
    ...
    else
    {
        ...
        if (desc.Raw)
	    {
            ...
            srvDesc.Buffer.NumElements = desc.NumElements > 0 ? desc.NumElements / 4 : (uint32_t)(bufferDesc.Size / 4);
...
void UnorderedAccessView::Create(GraphicsBuffer* pBuffer, const BufferUAVDesc& desc)
{
    ...
    if (desc.Counter)
    {
        ...
        m_pCounter = GetParent()->GetGraphics()->CreateBuffer(BufferDesc::CreateByteAddress(4), name.c_str());
    }
