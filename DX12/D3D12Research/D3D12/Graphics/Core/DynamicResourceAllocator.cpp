...
constexpr static uint64_t PAGE_SIZE = Math::MegabytesToBytes * 2;
...
void DynamicAllocationManager::FreeLargePages(uint64_t fenceValue, const std::vector<Buffer*> pLargePages)
{
	...
	while (m_DeleteQueue.size() > 0 && GetGraphics()->IsFenceComplete(m_DeleteQueue.front().first))
	{
		m_DeleteQueue.pop();
	}
...
void DynamicAllocationManager::CollectGrabage()
{
