...
{
	uint32_t width = D3D::IsBlockCompressFormat(textureDesc.Format) ? Math::Clamp(textureDesc.Width, 0u, textureDesc.Width) : textureDesc.Width;
	uint32_t height = D3D::IsBlockCompressFormat(textureDesc.Format) ? Math::Clamp(textureDesc.Height, 0u, textureDesc.Height) : textureDesc.Height;
	...
	case TextureDimension::Texture3D:
		...
		desc.DepthOrArraySize = (UINT16)textureDesc.DepthOrArraySize;
		break;
	default:
		noEntry();
		break;
...
void GraphicsTexture::Create(const TextureDesc& textureDesc)
{
	...
	m_pResource = GetGraphics()->CreateResource(desc, GetResourceState(), D3D12_HEAP_TYPE_DEFAULT, pClearValue);
	...
	if (EnumHasAnyFlags(textureDesc.Usage, TextureFlag::RenderTarget))
	{
		if (m_Rtv.ptr == 0)
		{
			m_Rtv = GetGraphics()->AllocateDescriptor<D3D12_RENDER_TARGET_VIEW_DESC>();
		}
		...
		GetGraphics()->GetDevice()->CreateRenderTargetView(m_pResource, &rtvDesc, m_Rtv);
	}
	else if (EnumHasAnyFlags(textureDesc.Usage, TextureFlag::DepthStencil))
	{
		if (m_Rtv.ptr == 0)
		{
			m_Rtv = GetGraphics()->AllocateDescriptor<D3D12_DEPTH_STENCIL_VIEW_DESC>();
		}

		if (m_ReadOnlyDsv.ptr == 0)
		{
			m_ReadOnlyDsv = GetGraphics()->AllocateDescriptor<D3D12_DEPTH_STENCIL_VIEW_DESC>();
		}
		...
		GetGraphics()->GetDevice()->CreateDepthStencilView(m_pResource, &dsvDesc, m_Rtv);
...
void GraphicsTexture::CreateForSwapChain(ID3D12Resource* pTexture)
{
	...
	GetGraphics()->GetDevice()->CreateRenderTargetView(pTexture, nullptr, m_Rtv);
