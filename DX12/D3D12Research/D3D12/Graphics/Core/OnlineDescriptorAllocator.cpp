...
DescriptorHeapBlock* GlobalOnlineDescriptorHeap::AllocateBlock()
{
    ...
    for (uint32_t i = 0; i < (uint32_t)m_ReleasedBlocks.size(); i++)
    {
...
OnlineDescriptorAllocator::OnlineDescriptorAllocator(GlobalOnlineDescriptorHeap* pGlobalHeap)
    : GraphicsObject(pGlobalHeap->GetGraphics()), m_Type(pGlobalHeap->GetType()), m_pHeapAllocator(pGlobalHeap)
{}
...
void OnlineDescriptorAllocator::ParseRootSignature(RootSignature* pRootSignature)
{
    ...
    const std::array<uint32_t, MAX_NUM_ROOT_PARAMETERS>& descriptorTableSizes = pRootSignature->GetDescriptorTableSizes();
...
void PersistentDescriptorAllocator::Free(int32_t& heapIndex)
{
	std::lock_guard lock(m_AllocationLock);
