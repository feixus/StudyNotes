...
public:
	...
	void RenderUI();
	void Render(Graphics* pGraphics, GraphicsTexture2D* pSceneTexture, GraphicsTexture2D* pDepthTexture);

private:
	std::unique_ptr<ComputePipelineState> m_pWorleyNoisePS;
	std::unique_ptr<RootSignature> m_pWorleyNoiseRS;
	std::unique_ptr<GraphicsTexture3D> m_pWorleyNoiseTexture;

	std::unique_ptr<GraphicsPipelineState> m_pCloudsPS;
	std::unique_ptr<RootSignature> m_pCloudsRS;

	std::unique_ptr<GraphicsTexture2D> m_pIntermediateColor;
	std::unique_ptr<GraphicsTexture2D> m_pIntermediateDepth;
