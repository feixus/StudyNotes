...
PersistentResourceAllocatorHeap::PersistentResourceAllocatorHeap(ID3D12Device* pDevice, D3D12_HEAP_FLAGS flags, uint64_t heapSize)
...
ID3D12Resource* PersistentResourceAllocatorHeap::CreateResource(const D3D12_RESOURCE_DESC& desc, D3D12_RESOURCE_STATES initialState, D3D12_CLEAR_VALUE* clearVlaue)
{
	D3D12_RESOURCE_ALLOCATION_INFO info = m_pDevice->GetResourceAllocationInfo(0, 1, &desc);
	uint64_t offset = Math::AlignUp(m_CurrentOffset, info.Alignment);
