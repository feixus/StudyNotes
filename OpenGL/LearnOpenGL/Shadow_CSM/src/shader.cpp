...
Shader::Shader(const char* a_vertexShaderSource, const char* a_fragmentShaderSource, const char* a_gsShaderSource)
{
    setVertexShader(a_vertexShaderSource);
    setFragmentShader(a_fragmentShaderSource);
...
{
    glUniform4fv(glGetUniformLocation(shaderProgram, name.c_str()), 1, &vector[0]);
}
...
{
    glUniform1i(glGetUniformLocation(shaderProgram, name.c_str()), variable);
}