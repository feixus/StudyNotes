...
namespace My
{
    ...
    class TestApplication : public My::WindowsApplication
    {
    public:
...
{
    int result = WindowsApplication::Initialize();
...
void My::TestGraphicsManager::DrawBitmap(const Image image[], int32_t index)
{
    ...
    D2D1_RECT_F source_rect = D2D1::RectF(0.0f, 0.0f, bmpSize.width, bmpSize.height);
    float aspect = bmpSize.width / bmpSize.height;
    float dest_height = rtSize.height;
    float dest_width = dest_height * aspect;
